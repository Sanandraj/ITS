 ...
 * Load Code Extension to N4:
 ...
 Code Extension Name:  ITSTMFReleasePostInterceptor
...
class ITSTMFReleasePostInterceptor extends AbstractEdiPostInterceptor {
    private static final Logger LOGGER = Logger.getLogger(ITSTMFReleasePostInterceptor.class)

    void beforeEdiPost(XmlObject inXmlTransactionDocument, Map inParams) {
        //LOGGER.setLevel(Level.DEBUG)
        LOGGER.debug("ITSTMFReleasePostInterceptor - beforeEdiPost - Execution started.")
        ...
        LOGGER.debug("ITSTMFReleasePostInterceptor - beforeEdiPost - Execution completed.")
    }
    ...
    private CarrierVisit findEdiVesselVisit(EdiVesselVisit inEdiVesselVisit, String inReleaseIdType) throws BizViolation {
        ...
        if (inEdiVesselVisit != null && inReleaseIdType != null) {
            LOGGER.debug("ITSTMFReleasePostInterceptor - findEdiVesselVisit - inReleaseIdType: " + inReleaseIdType)
            ...
            if (RELEASE_TYPE_UNIT.equalsIgnoreCase(inReleaseIdType)) {
                ...
                LOGGER.debug("ITSTMFReleasePostInterceptor - findEdiVesselVisit - Booking cv : " + cv)
                return cv
            }
    ...
    private void logMsg(String inMsg) {
        LOGGER.debug("ITSTMFReleasePostInterceptor : " + inMsg)
    }
