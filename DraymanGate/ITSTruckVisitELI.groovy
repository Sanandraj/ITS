        ...
        try {
            ...
            if (library) {
                ...
                if ( inOriginalFieldChanges.hasFieldChange(RoadField.TVDTLS_NEXT_STAGE_ID)
                        && T__INGATE == (String)inOriginalFieldChanges.findFieldChange(RoadField.TVDTLS_NEXT_STAGE_ID).getPriorValue()
                        && T__YARD   == (String)inOriginalFieldChanges.findFieldChange(RoadField.TVDTLS_NEXT_STAGE_ID).getNewValue()
                        && TruckVisitStatusEnum.OK.equals(truckVisitDetails.getTvdtlsStatus()) ) {

                    LOGGER.debug("process at InGate stage");
                    /*String messageType = T__PICKUP;
                    for (TruckTransaction tran : truckVisitDetails.getTransactionsInProgress()) {
                        if (tran.isReceival()) {
                            messageType = T__SITE_ARRIVAL;
                            break;
                        }
                    }*/
                ...
                /*if (TruckVisitStatusEnum.CANCEL == inOriginalFieldChanges.findFieldChange(RoadField.TRAN_STATUS).getNewValue()
                 || TruckVisitStatusEnum.COMPLETE == inOriginalFieldChanges.findFieldChange(RoadField.TRAN_STATUS).getNewValue()) {*/
                if ( inOriginalFieldChanges.hasFieldChange(RoadField.TRAN_STATUS)
                        && TruckVisitStatusEnum.COMPLETE != inOriginalFieldChanges.findFieldChange(RoadField.TVDTLS_STATS).getPriorValue()
                        && TruckVisitStatusEnum.COMPLETE == inOriginalFieldChanges.findFieldChange(RoadField.TVDTLS_STATS).getNewValue() ) {
