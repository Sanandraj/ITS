 ...
 * Date: 19/01/2022
 * Requirements:- Receives one or more Chassis Number(s) via Http Get & Returns Chassis Event Histories in JSON format
 *  @Inclusion Location	: Incorporated as a code extension of the type REQUEST_SIMPLE_READ --> Paste this code (ITSGetChassisEventHistoryWS.groovy)
 */


class ITSGetChassisEventHistoryWS extends AbstractSimpleRequest {
    @Override
    String execute(UserContext paramUserContext, Map paramMap) {
        ...
        MessageCollector collector = handler.executeInTransaction(paramUserContext, FrameworkExtensionTypes.TRANSACTED_BUSINESS_FUNCTION, "ITSGetChassisEventHistoryWSCallback", input, outPut)
