 ...
 * Date: 15/02/2022
 * Requirements:- Receives one or more Booking Number(s) via Http Get and returns a list of Booking detals in JSON format.
 *  @Inclusion Location	: Incorporated as a code extension of the type REQUEST_SIMPLE_READ --> Paste this code (ITSGetBookingWS.groovy)
 */


class ITSGetBookingWS extends AbstractSimpleRequest {
    @Override
    String execute(UserContext paramUserContext, Map paramMap) {
        ...
        MessageCollector collector = handler.executeInTransaction(paramUserContext, FrameworkExtensionTypes.TRANSACTED_BUSINESS_FUNCTION, "ITSGetBookingWSCallback", input, outPut)
