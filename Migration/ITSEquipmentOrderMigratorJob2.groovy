...
class ITSEquipmentOrderMigratorJob2 extends AbstractGroovyJobCodeExtension {
    ...
    private void updateHolds(Booking booking, JSONObject jsonObj){
        ...
        /*if (StringUtils.isNotEmpty(port_count)) {
            ...
            if (portCount > 0) {
                List<JSONArray> portHold = (List<JSONArray>) jsonObj.getOrDefault("port-hold", null)
                if (portHold != null && portHold.size() > 0) {
                    JSONArray jarray = portHold.get(0)
                    for (int i = 0; i < jarray.size(); i++) {
                        JSONObject portFeeObj = jarray.getJSONObject(i)
                        if (portFeeObj != null) {
                            String release_code = portFeeObj.getOrDefault("release_code", null)
                            String performed = portFeeObj.getOrDefault("performed", null)
                            String updated_on = portFeeObj.getOrDefault("updated_on", null)
                            ...
                            if(StringUtils.isNotEmpty(release_code) ){
                                applyHold("PORT FEE HOLD EXP", booking, null, null, pfDate, 'Apply hold during migration');
                                if("R".equalsIgnoreCase(release_code)){
                                    releaseHold("PORT FEE HOLD EXP", booking, null, null, pfDate, 'Released during migration');
        ...
        String tmf_count = jsonObj.getOrDefault("tmf_count", null)
        if (StringUtils.isNotEmpty(tmf_count)) {
            int tmfCount = Integer.valueOf(tmf_count)

            if (tmfCount > 0) {
                List<JSONArray> tmfHold = (List<JSONArray>) jsonObj.getOrDefault("tmf-hold", null)
                if (tmfHold != null && tmfHold.size() > 0) {
                    JSONArray jarray = tmfHold.get(0)
                    for (int i = 0; i < jarray.size(); i++) {
                        JSONObject tmfFeeObj = jarray.getJSONObject(i)
                        if (tmfFeeObj != null) {
                            String release_code = tmfFeeObj.getOrDefault("release_code", null)
                            String performed = tmfFeeObj.getOrDefault("performed", null)
                            Date pfDate = DateUtil.getTodaysDate(ContextHelper.getThreadUserContext().getTimeZone())
                            if (performed) {
                                pfDate = DateUtil.dateStringToDate(DateUtil.parseStringToDate(performed, ContextHelper.getThreadUserContext()).format('yyyy-MM-dd HH:mm:ss'))
                            }
                            String updated_on = tmfFeeObj.getOrDefault("updated_on", null)
                            if(StringUtils.isNotEmpty(release_code) ){
                                applyHold("TMF BKG", booking, null, null, pfDate, 'Apply hold during migration');
                                if("R".equalsIgnoreCase(release_code)){
                                    releaseHold("TMF BKG", booking, null, null, pfDate, 'Released during migration');
                    ...
                    for (int i = 0; i < jarray.size(); i++) {
                        ...
                        if (hazObj != null) {
                            ...
                            try {
                                ...
                                Boolean mpFlag = (marine_pollutant && (marine_pollutant == 'Y'|| marine_pollutant == 'X'));
