 ...
 *
 *  Requirements: This groovy is used for maintaining the inventory for all the common methods that needs to be called from majority of the Groovy classes.
 *
 ...
 *     Code Extension Name: BaseGroovyUtil
...
class BaseGroovyUtil {
    ...
    public void recordEventOnCompleteServiceOrder(String inEventTypeId, Unit inUnit, ServiceOrder serviceOrder, ScopedBizUnit billingParty, Double quantity) {
        ...
        if (quantity > 1) {
            currentEvent.setEvntQuantity(quantity)
            currentEvent.setEvntQuantityUnit(ServiceQuantityUnitEnum.UNITS)
        }
        try {
